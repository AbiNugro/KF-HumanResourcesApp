        ...
        <section class="section">
            <div class="card">
                <div class="card-header">
                    <h5 class="card-title">Update Presence Manually</h5>
                </div>
                <div class="card-body">
                    ...
                    <form action="{{ route('presences.update', $presence->id) }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="employee_id" id="employee_id" class="form-control @error('employee_id') is-invalid @enderror">
                                ...
                                <option value="{{ $employee->id }}" {{ ($employee->id == $presence->employee_id ? 'selected' : '') }}>{{ ucfirst($employee->fullname) }}</option>
                                @endforeach
                            </select>
                            @error('employee_id')
                        ...
                        <div class="mb-3">
                            ...
                            <input type="datetime" class="form-control datetime_flatpicker" name="check_in" value="{{ old('check_in', $presence->check_in) }}" required>
                            @error('check_in')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="datetime" class="form-control datetime_flatpicker" name="check_out" value="{{ old('check_out', $presence->check_out) }}" required>
                            @error('check_out')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
