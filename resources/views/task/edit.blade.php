                ...
                <div class="card-body">
                    ...
                    <form action="{{ route('tasks.update', $task->id) }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="date" class="form-control date_flatpicker @error('due_date') is-invalid @enderror" value="{{ @old('due_date', $task->due_date) }}" name="due_date" required>
                            @error('due_date')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <select name="status" id="status" class="form-control @error('status') is-invalid @enderror">
                                ...
                                <option value="progress" @if(old('status', $task->status) == 'progress') selected @endif>Progress</option>
                            </select>
                            @error('status')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="description" id="" class="form-control @error('description') is-invalid @enderror">{{ old('description', $task->description) }}</textarea>
                            @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
